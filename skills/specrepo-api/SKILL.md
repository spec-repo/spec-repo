...
**사용법**:
- `/specrepo-api draft` — 요구사항 + 아키텍처 → API명세서.md 초안 생성
- `/specrepo-api import <file>` — 기존 PDF/docx/yaml → MD 구조화
- `/specrepo-api sync <openapi-file>` — OpenAPI(Swagger) YAML/JSON → MD 갱신
- `/specrepo-api export [--format pdf|docx]` — MD → 산출물 파일 생성
- `/specrepo-api status` — 현황 확인
...
아래 순서로 파일을 탐색한다:
1. `references/01-requirements/requirements.md` — 기능 요구사항 (API 연관 항목)
2. `references/02-design/아키텍처설계서.md` — 기술 스택, Base URL, 인증 방식
3. `references/00-rfp/*/index.md` — RFP에서 API 관련 요건 (없으면 skip)
...
`skills/specrepo-api/data/API명세서-template.md`를 Read해서 구조를 파악한 후, 프로젝트에 맞게 내용을 채워 `references/02-design/API명세서.md`를 생성한다.
