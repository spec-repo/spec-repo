...
**사용법**:
- `/specrepo-db draft` — 요구사항 + 아키텍처 → DB설계서.md 초안 생성
- `/specrepo-db import <file>` — 기존 PDF/docx → MD 구조화
- `/specrepo-db export [--format pdf|docx]` — MD → 산출물 파일 생성
- `/specrepo-db ddl [--dialect mysql|oracle|postgresql]` — MD → DDL 생성
- `/specrepo-db status` — 현황 확인
...
`skills/specrepo-db/data/DB설계서-template.md`를 Read해서 구조를 파악한 후, 프로젝트에 맞게 내용을 채워 `references/02-design/DB설계서.md`를 생성한다.
...
```bash
# DDL 생성 (oracle 기본)
uv run python3 skills/specrepo-db/scripts/md-to-ddl.py \
