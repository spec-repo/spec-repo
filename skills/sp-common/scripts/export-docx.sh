#!/usr/bin/env bash
# export-docx.sh — 마크다운 → Word(.docx) 변환 (pandoc + Mermaid 전처리)
#
# 사전 요건:
...
#
# 사용법:
#   ./scripts/export-docx.sh <문서명> [버전] [--template <template.docx>]
#
# 예시:
#   ./scripts/export-docx.sh 아키텍처설계서
#   ./scripts/export-docx.sh 아키텍처설계서 1.0.0
#   ./scripts/export-docx.sh 아키텍처설계서 1.0.0 --template references/02-design/_template.docx
...
if ! command -v pandoc &>/dev/null; then
  ...
  echo "설치: sudo apt install pandoc  (Ubuntu/WSL)"
  exit 1
fi
...
[ -z "$DOC_NAME" ] && { echo "사용법: $0 <문서명> [버전] [--template <template.docx>]"; exit 1; }
...
docx_file="$SNAPSHOTS_DIR/${stage_sub:-misc}/${DOC_NAME}${ver_suffix}_${TODAY}.docx"

echo "변환 중: ${DOC_NAME}.md → snapshots/$(basename "$docx_file")"
...
if [ -z "$TEMPLATE_ARG" ]; then
  ...
  if [ -f "$doc_dir/_template.docx" ]; then
    TEMPLATE_ARG="$doc_dir/_template.docx"
